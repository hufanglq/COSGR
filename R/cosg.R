...
cosg<-function(
  ...
  if(!inherits(genexcell, "dgCMatrix"))
    genexcell <- as(as.matrix(genexcell), "dgCMatrix")
  ...
  cosine_sim=suppressWarnings(
    proxyC::simil(genexcell,cluster_mat_sparse, method = "cosine",drop0=TRUE)
  )
  ...
  for (group_i in groups_order){
    ...
    if(remove_lowly_expressed){
      ...
      n_cells_expressed=tabulate(genexcell[,idx_i]@i + 1, nrow(genexcell[,idx_i]))
      n_cells_i=sum(idx_i)
